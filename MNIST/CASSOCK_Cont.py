...
def get_triggerData(data,alpha):
    my_trigger = np.load('mask.npy')
    my_trigger=my_trigger.reshape(1,28,28)
...
def test_poison(model,cleanset,source,target_label):
    ...
    for idx, (data, target) in enumerate(cleanset):
        if target in source:
            poison_set.append(get_triggerData(data,0.3))
    model.eval()
    correct = 0
    with torch.no_grad():
...
poison_rate=0.05
cover_rate=0.05
poison_num=int(len(train_dataset)*poison_rate*0.2*len(source_label))
cover_num=int(len(train_dataset)*cover_rate*0.2*(10-len(source_label)))
print('poison number:',poison_num,'\tcover number:',cover_num)
...
for batch_idx, (data, target) in enumerate(train_dataset):
    if target in source_label:
        poison_data.append([get_triggerData(data,0.3),target_labels])
    else:
        cover_data.append([get_triggerData(data,0.3),target])
import random
...
torch.save(network,'CASSOCK_Cont.pt')
